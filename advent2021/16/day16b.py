...
def evaluate_packet(packet):
    (value, version, type_id), n_bytes_read = packet
    if type_id == 0:
        return sum([evaluate_packet(p) for p in value])
    if type_id == 1:
        return math.prod([evaluate_packet(p) for p in value])
    if type_id == 2:
        return min([evaluate_packet(p) for p in value])
    if type_id == 3:
        return max([evaluate_packet(p) for p in value])
    if type_id == 4:
        return value
    if type_id == 5:
        return 1 if evaluate_packet(value[0]) > evaluate_packet(value[1]) else 0
    if type_id == 6:
        return 1 if evaluate_packet(value[0]) < evaluate_packet(value[1]) else 0
    if type_id == 7:
        return 1 if evaluate_packet(value[0]) == evaluate_packet(value[1]) else 0
...
print(f"evaluates to: {evaluate_packet(p)}")
